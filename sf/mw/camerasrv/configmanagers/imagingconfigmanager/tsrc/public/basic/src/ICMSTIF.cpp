...
TInt CICMSTIF::TestCase_002L( TTestResult& aResult )
	{
	CImagingConfigManager* icm = CImagingConfigManager::NewL();
    if(icm->NumberOfVideoQualityLevels() != 8)
        {
        aResult.iResult = KErrGeneral;
		aResult.iResultDes = _L("NumberOfVideoQualityLevels() didn't return 8 as expected.");
        }
    else
        {
        aResult.iResult = KErrNone;
...
TInt CICMSTIF::TestCase_003L( TTestResult& aResult )
    ...
    if(levels->Count() != 8 || levels->At(0) != 98
                             || levels->At(1) != 99 || levels->At(2) != 200
                             || levels->At(3) != 201 || levels->At(4) != 202 || levels->At(5) != 203
                             || levels->At(6) != 301 || levels->At(7) != 302)
        {
        aResult.iResult = KErrGeneral;
		aResult.iResultDes = _L("GetVideoQualityLevelsL() didn't return the expected values.");
        }
    else
        {
        icm->GetVideoQualitySet( videoset, 100/*levels->At(0)*/ );
...
TInt CICMSTIF::TestCase_007L( TTestResult& aResult )
    ...
    if(levels->Count() != 24 || levels->At(0) != 99 || levels->At(1) != 100 || levels->At(2) != 101
        ...
        {
        aResult.iResult = KErrGeneral;
		aResult.iResultDes = _L("GetImageQualityLevelsL() didn't return expected value.");
        }
    else
        {
        ...
        if( memcompare(imageset.iImageFileMimeType, length1, KImageFileMimeType().Ptr(), KImageFileMimeType().Length()) ||
            ...
            imageset.iImageWidth != 640 ||
            imageset.iImageHeight != 480 ||
            imageset.iImageEncoderUID.iUid != 0 ||
            imageset.iCompressionQuality != 50 ||
            imageset.iEstimatedSize != 28590 ||
            imageset.iCameraDisplayId != 2 ||
            imageset.iCamcorderVisible != 100)
            {
            aResult.iResult = KErrGeneral;
...
TInt CICMSTIF::TestCase_008L( TTestResult& aResult )
    ...
    if(imageset.iImageHeight != resolution.iHeight ||
       ...
       imageset.iImageQualitySetLevel != 103)
        {
        aResult.iResult = KErrGeneral;
		aResult.iResultDes = _L("GetImageQualitySetL() didn't return expected value.");
        }
    else
        {
        TInt length1 = KMaxStringLength;
        ...
        if( memcompare(imageset.iImageFileMimeType, length1, KImageFileMimeType().Ptr(), KImageFileMimeType().Length()) ||
            ...
            imageset.iImageEncoderUID.iUid != 0 ||
            imageset.iCompressionQuality != 50 ||
            imageset.iEstimatedSize != 114370 ||
            imageset.iCameraDisplayId != 2 ||
            imageset.iCamcorderVisible != 0)
            {
            aResult.iResult = KErrGeneral;
...
TInt CICMSTIF::TestCase_010L( TTestResult& aResult )
    ...
    icm->GetImageQualitySet( imageset, 305 );
    TInt length1 = KMaxStringLength;
    TInt length2 = KMaxStringLength;
    for( TInt i=0; i<KMaxStringLength; i++)
        {
    ...
    if( memcompare(imageset.iImageFileMimeType, length1, KImageFileMimeType().Ptr(), KImageFileMimeType().Length()) ||
        memcompare(imageset.iImageFileExtension, length2, KImageFileExtension().Ptr(), KImageFileExtension().Length()) ||
        imageset.iImageWidth != 2048 ||
        imageset.iImageHeight != 1536 ||
        imageset.iImageEncoderUID.iUid != 0 ||
        imageset.iCompressionQuality != 85 ||
        imageset.iEstimatedSize != 430100 ||
        imageset.iCameraDisplayId != 2 ||
        imageset.iCamcorderVisible != 300)
        {
        aResult.iResult = KErrGeneral;
	    aResult.iResultDes = _L("GetImageQualityLevelsL() didn't return expected values.");
        }
