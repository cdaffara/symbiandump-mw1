...
#define _ABS_SIMPLE_CONFIG_BASE_RECORD_H_
...
public:
	...
	/**
	 * The destructor of the abs_simple_config_base_record_c class does nothing special.
	 */
	virtual ~abs_simple_config_base_record_c()
	{
	}

	/**
	 * The constructor of the abs_simple_config_base_record_c class does nothing special.
	 */
	 ...
	 * @param message_type is the type of the SIMPLE_CONFIG message indicated by the higher layer.
	 */
	virtual eap_status_e simple_config_packet_send(
		eap_buf_chain_wr_c * const sent_packet,
		const simple_config_Message_Type_e message_type) = 0;

	/**
	 * The get_header_offset() function obtains the header offset of SIMPLE_CONFIG-packet.
	 * @param MTU_length is pointer to variable to store the maximum transfer unit (MTU).
	 * MTU is the maximum SIMPLE_CONFIG-packet length in bytes
	 * @param trailer_length is pointer to the variable to store length
	 * of trailer needed by lower levels.
	 * @return Function returns the offset of SIMPLE_CONFIG-header.
	 *
	 * The needed buffer length is ((offset) + (SIMPLE_CONFIG-packet length) + (trailer)) bytes.
	 ...
	 * through adaptation module of EAP-type. See eap_am_type_simple_config_simulator_c::type_configure_read.
	 */
	 ...
	 * 
	 * EAP-type should store it's parameters to an own database. The own database should be accessed
	 * through adaptation module of EAP-type. See eap_am_type_simple_config_simulator_c::type_configure_write.
	 */
...
#endif //#if !defined(_ABS_SIMPLE_CONFIG_BASE_RECORD_H_)
