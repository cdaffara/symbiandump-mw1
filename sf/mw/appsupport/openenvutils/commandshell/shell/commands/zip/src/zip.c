...
{
    zip_internal *ziinit = NULL;  //changed to heap_alloc
    zip_internal* zi = NULL;
    int err=ZIP_OK;

    ziinit = (zip_internal*)ALLOC(sizeof(zip_internal));
    if (ziinit==NULL)
	{
        return NULL;
    }
...
#    ifndef NOCRYPT
    ...
    {
        unsigned char bufHead[RAND_HEAD_LEN]; //heap_alloc not needed- 12 bytes only.
...
#	 else
    if (crcForCrypting  == 0) { /* do nothing */ }
#    endif
