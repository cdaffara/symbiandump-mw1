...
*/
CEASShdOpCodeGetAlarmSoundState1Step::CEASShdOpCodeGetAlarmSoundState1Step()
	{
...
*/
TInt CEASShdOpCodeGetAlarmSoundState1Step::Exec_SendReceiveL()
	{
	...
	if(iResultServer == 0)
		...
		if(iSrMessageId >= 0)
			{
			TPckgBuf<TAlarmGlobalSoundState> state;
			iResultSr =	SendReceive(iSrMessageId,TIpcArgs(&state));
			}
		}
	return iResultServer;
	}
