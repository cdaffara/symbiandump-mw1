...
*/
CEASShdOpCodeSetAlarmStatus_DifferentSID1Step::CEASShdOpCodeSetAlarmStatus_DifferentSID1Step()
	{
...
*/
TInt CEASShdOpCodeSetAlarmStatus_DifferentSID1Step::Exec_SendReceiveL()
	{
	...
	if(iResultServer == 0)
		...
		if(iSrMessageId >= 0)
			...
			if(alarmId >= 0)
				...
				TAlarmStatus status = EAlarmStatusDisabled;
				TIpcArgs args(alarmId, status);
