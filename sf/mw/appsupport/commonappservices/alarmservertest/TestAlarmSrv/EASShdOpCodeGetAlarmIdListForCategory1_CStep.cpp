...
*/
CEASShdOpCodeGetAlarmIdListForCategory1Step::CEASShdOpCodeGetAlarmIdListForCategory1Step()
	{
...
*/
TInt CEASShdOpCodeGetAlarmIdListForCategory1Step::Exec_SendReceiveL()
	{
	...
	if(iResultServer == 0)
		...
		if(iSrMessageId >= 0)
			{
			TPckgBuf<TAlarmCategory> cat;
			TPckgBuf<TInt> size;
			iResultSr =	SendReceive(iSrMessageId,TIpcArgs(&cat, &size));
			}
		}
	return iResultServer;
	}
