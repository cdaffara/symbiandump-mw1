...
*/
CEASShdOpCodeGetNextDueAlarmId1Step::CEASShdOpCodeGetNextDueAlarmId1Step()
	{
...
*/
TInt CEASShdOpCodeGetNextDueAlarmId1Step::Exec_SendReceiveL()
	{
	...
	if(iResultServer == 0)
		...
		if(iSrMessageId >= 0)
			{
			TPckgBuf<TAlarmId> id;
			iResultSr =	SendReceive(iSrMessageId,TIpcArgs(&id));
			}
		}
	return iResultServer;
	}
