...
_LIT(KTestApp1,"tamastarter_app1");
_LIT(KTestApp2,"tamastarter_app2");
_LIT(KTestApp3,"tamastarter_app3");
...
void StartTestL(void)
	...
	CleanupStack::PopAndDestroy(2); //cmdLine, appResults
	...
	CleanupStack::PopAndDestroy(); //testAmaStarterLogMutex
	TTime endtime;
	...
	TInt appNumber = 0;
	if(fileForLog.Compare(KTestApp1) == 0)
		{
		appNumber = 1;
		}
	else if(fileForLog.Compare(KTestApp2) == 0)
		{
		appNumber = 2;
		}
	else if(fileForLog.Compare(KTestApp3) == 0)
		{
		appNumber = 3;
		}
	RProperty::Set(KTestAmastartCategory, KTestAmastartKey, appNumber);
	}
