/****************************************************************************
**
** Copyright (C) 2008-2010 Nokia Corporation and/or its subsidiary(-ies).
** All rights reserved.
** Contact: Nokia Corporation (developer.feedback@nokia.com)
**
** This file is part of the HbCore module of the UI Extensions for Mobile.
**
** GNU Lesser General Public License Usage
** This file may be used under the terms of the GNU Lesser General Public
** License version 2.1 as published by the Free Software Foundation and
** appearing in the file LICENSE.LGPL included in the packaging of this file.
** Please review the following information to ensure the GNU Lesser General
** Public License version 2.1 requirements will be met:
** http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html.
**
** In addition, as a special exception, Nokia gives you certain additional
** rights.  These rights are described in the Nokia Qt LGPL Exception
** version 1.1, included in the file LGPL_EXCEPTION.txt in this package.
**
** If you have questions regarding the use of this file, please contact
** Nokia at developer.feedback@nokia.com.
**
****************************************************************************/

#include "hbvgblureffect_p.h"
#include "hbvgblureffect_p_p.h"
#include <QPainter>

/*!
 * \class HbVgBlurEffect
 *
 * \brief OpenVG-based blur filter effect.
 *
 * \internal
 */

HbVgBlurEffectPrivate::HbVgBlurEffectPrivate()
    : radius(0, 0)
{
}

HbVgBlurEffect::HbVgBlurEffect(QObject *parent)
    : HbVgEffect(*new HbVgBlurEffectPrivate, parent)
{
}

HbVgBlurEffect::HbVgBlurEffect(HbVgBlurEffectPrivate &dd, QObject *parent)
    : HbVgEffect(dd, parent)
{
}

HbVgBlurEffect::~HbVgBlurEffect()
{
}

QPointF HbVgBlurEffect::radius() const
{
    Q_D(const HbVgBlurEffect);
    return d->radius;
}

void HbVgBlurEffect::setRadius(const QPointF &radius)
{
    Q_D(HbVgBlurEffect);
    if (d->radius == radius) {
        return;
    }
    d->radius = radius;
    updateEffectBoundingRect();
    emit radiusChanged(radius);
}

QRectF HbVgBlurEffect::boundingRectFor(const QRectF &rect) const
{
    Q_D(const HbVgBlurEffect);
    QSizeF mappedRadius = d->mapSize(QSizeF(d->radius.x(), d->radius.y()));
    qreal deltaX = mappedRadius.width();
    qreal deltaY = mappedRadius.height();
    return rect.adjusted(-deltaX, -deltaY, deltaX, deltaY);
}

#ifdef HB_EFFECTS_OPENVG
// identity mapping of colour components (i.e. 0->0, 1->1, ..., 255->255)
VGubyte identityLUT[256] = {
    
};
#endif

QPixmap HbVgBlurEffect::makeBlur(const QVariant &vgImage, const QSize &vgImageSize)
{
#ifdef HB_EFFECTS_OPENVG
    QPixmap cachedPm = cached(vgImageSize);
    if (!cachedPm.isNull()) {
        return cachedPm;
    }

    Q_D(HbVgBlurEffect);
    VGImage srcImage = vgImage.value<VGImage>();
    VGImage dstImage = d->ensurePixmap(&d->dstPixmap, vgImageSize);
    QSizeF mappedRadius = d->mapSize(QSizeF(d->radius.x(), d->radius.y()));
    const VGfloat blurX = (VGfloat) clamp(mappedRadius.width(), HBVG_EPSILON, 16.0f);
    const VGfloat blurY = (VGfloat) clamp(mappedRadius.height(), HBVG_EPSILON, 16.0f);
    const VGfloat opacity = (VGfloat) clamp(d->opacity, 0.0f, 1.0f);
    if (opacity < 1.0f - HBVG_EPSILON) {
        VGImage tmpImage = d->ensurePixmap(&d->tmpPixmap, vgImageSize);
        vgGaussianBlur(tmpImage, srcImage, blurX, blurY, VG_TILE_PAD);
        if (d->paramsChanged) {
            for (int i = 0; i < 256; ++i) {
                d->alphaLUT[i] = (VGubyte)(i * opacity);
            }
        }
        vgLookup(dstImage, tmpImage,
                 identityLUT, identityLUT, identityLUT, d->alphaLUT,
                 VG_TRUE, VG_FALSE);
    } else {
        vgGaussianBlur(dstImage, srcImage, blurX, blurY, VG_TILE_PAD);
    }

    tryCache(d->dstPixmap);
    return d->dstPixmap;
#else
    Q_UNUSED(vgImage);
    Q_UNUSED(vgImageSize);
    return QPixmap();
#endif
}

void HbVgBlurEffect::performEffect(QPainter *painter,
                                   const QPointF &offset,
                                   const QVariant &vgImage,
                                   const QSize &vgImageSize)
{
#ifdef HB_EFFECTS_OPENVG
    QPixmap blurred = makeBlur(vgImage, vgImageSize);
    painter->drawPixmap(offset, blurred);
#else
    Q_UNUSED(painter);
    Q_UNUSED(offset);
    Q_UNUSED(vgImage);
    Q_UNUSED(vgImageSize);
#endif
}
