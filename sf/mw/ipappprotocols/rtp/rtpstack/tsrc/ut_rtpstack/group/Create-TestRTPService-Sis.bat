...
copy %EPOCROOT%epoc32\release\thumb\urel\TestRTPService.dll .
if errorlevel == 1 goto END_ERROR 
...
:MAKESIS
echo.
echo - makesis TestRTPService.pkg ..\sis\TestRTPService.sis
call makesis TestRTPService.pkg ..\sis\TestRTPService.sis
if errorlevel == 1 goto END_ERROR

:DELETE_DLL
echo.
echo - remove the copied dll from current folder
del TestRTPService.dll
goto END
